...
Цель выбирается такой же одной оценкой `_best_target_for` (веса `TARGET_SCORE_*`):
ранг цели, сбитые корабли, снятая доля прочности, добивание пачки, цена похода
и — дороже всего остального — возможность отработать по ней прямо в этот ход.
Низкий ранг остаётся главным ориентиром, но это слагаемое, а не вето: раньше
ранг сравнивался первым и строго, и охотник уходил от добиваемой пачки под
боком через всё поле за целой пачкой рангом ниже.

Достижимость цели и возможность залпа обе стороны меряют ровно теми же
функциями, которыми залп потом и состоится: `_reachable_cells` (BFS в обход
препятствий и чужих корпусов, тот же, что у `_best_enemy_move_cell`) и
`_attack_cell_from` (дальность, слепая зона патруля `min_engage_range`, линия
огня, оба гекса корпуса цели IV+ ранга). Прямая `_hex_distance` до головного
гекса для этого не годится.
...
3. `MOVE_SCORE_HOLD` — бонус клетке, на которой пачка УЖЕ стоит. До этого тут
   был обратный по смыслу `MOVE_SCORE_REPOSITION` (бонус за сам факт манёвра),
   и отряд вплотную к цели каждый ход прыгал между двумя одинаковыми соседними
   гексами. Манёвр перед залпом никуда не делся: пачка сходит, как только
   клетка даёт больше урона, меньше ответки или выводит из клещей.
4. `TARGET_SCORE_KEEP` — гистерезис смены цели, `unit["ai_target"]` хранит
   прежний выбор. Без него отряд разворачивался на полпути каждый раз, когда
   союзники подранивали соседнюю равноранговую пачку, и не доходил ни до одной.
   Обнулять `ai_target` при правках очереди хода не нужно: мёртвая цель просто
   перестаёт получать надбавку.
