...
**Пятая редакция первой миссии (24.09.2026):** добавлены 11 необязательных
встречных флотов пиратов и торговцев в разных секторах, новые станции
разнесены по карте; круги станций и командующих убраны, радиусы перехвата
пиратов и патрулей видны вокруг кораблей. «Сломанный маяк» удалён из
игры. Подробности — `data/campaign/README.md`.
...
| `docs/android.md` | сенсорное управление, параметры и сборка Android APK |
...
| `tools/test_android_controls.gd` | касания карты и боя, свайп, масштаб и системная кнопка «Назад» |
...
* **Отступы интерфейса** — текст и кнопки не прижимать к краям окон и карточек:
  у модальных окон оставлять не меньше 24 px внутреннего поля, у карточек —
  не меньше 12 px. Проверять реальные снимки экранов после изменения вёрстки.
...
Локализации пока нет. Марсианские бандиты — противник кампании под управлением
`bandit_ai.gd`. В случайной партии Марс доступен игроку и использует общий
экран планеты с панорамами `assets/planet_surface/mars/town`.
Устаревший отдельный экран базы и его плейсхолдеры удалены.
Озвучки UI нет. `data/ship_configs/` пустой — редактор
