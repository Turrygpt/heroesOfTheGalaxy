...
существующие корабли людей и марсианских бандитов), детальная панельная обшивка, турели/
...
- **Марсианские бандиты** — тёмный металл, агрессивный красный (`ENEMY_COLOR` в коде).
...
процедурная заглушка по образцу `tools/make_bandit_placeholders.py`). Промты
...
стороны (своя база, экономика, ход ИИ — модуль уровня `bandit_ai.gd`,
