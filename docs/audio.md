...
Боевой синтез различает очередь истребителя, кинетическую пушку, луч, ракету,
плазменный разряд, двигатель, попадание в корпус и силовое поле, уничтожение.
Размер звучания берётся из ранга корабля (для старых записей — из корпуса),
поэтому бонус к ХП не меняет тембр. На каждый тип и размер есть три варианта.
При входе в бой нужные буферы готовятся в отдельном потоке, а редкий звук,
который не успели подготовить, синтезируется по запросу. Синтез использует
собственный генератор случайных чисел и не сдвигает броски боевой системы.
Голоса идут через дочернюю шину `CombatSFX` с ограничителем пиков, затем через
обычную `SFX`, так что настройка громкости действует и на них. Звук попадания
задерживается до прилёта снаряда; промах и быстрый расчёт молчат.
`tools/test_combat_audio.gd --capture` проверяет сигналы и сохраняет
`build/combat_audio/combat_demo.wav` с короткой подборкой звуков.
