...
`index.html` — страница демо, установщика Windows, тестового Android APK и отзывов. `feedback/index.html` — отдельная анкета, `admin/index.html` — модерация отзывов. Общий стиль — `style.css`; за анкету отвечает `app.js`, за отзывы — `reviews.js`, за админку — `admin.js`. В `deploy/` лежат конфигурация Nginx и служба systemd.
...
Кнопки Windows ведут через `/playtest/api/download`: сервер увеличивает счётчик версии и перенаправляет браузер к установщику. Админка показывает общую сумму и разбивку по версиям. Счётчик хранит только числа; IP, cookie и время в этой таблице не сохраняются. Он считает начало скачивания по кнопке, а не подтверждённое завершение передачи файла. Кнопка Android ведёт напрямую к APK и пока не входит в счётчик. Прямые ссылки на файлы и скачивания до включения счётчика в статистику не входят.
...
Android 0.12.0: файл `build/android/HeroesOfTheGalaxy-0.12.0-debug.apk`
публикуется как
`downloads/HeroesOfTheGalaxy-0.12.0-android-arm64-debug.apk`.
В `downloads/SHA256SUMS.txt` остаётся строка Windows и добавляется SHA-256
APK. Сначала загружать APK и суммы, затем `index.html` и `style.css`, после
чего проверить обе ссылки, размер файла и контрольную сумму на сайте.
