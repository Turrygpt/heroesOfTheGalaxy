...
## Раньше эти цвета были почти неотличимы от обычного фона карты (звёздный
## параллакс + полупрозрачная подложка сетки) - туман технически работал
## (маска открытых клеток считалась верно), но визуально не читался вообще,
## игрок не видел разницы между исследованной и неисследованной клеткой.
## Сделаны заметно темнее фона, почти чёрными, чтобы неисследованные клетки
## однозначно читались как туман, а не терялись среди фоновых туманностей.
const BASE_COLOR := Color(0.0, 0.0, 0.005, 1.0)
const NEBULA_COLOR := Color(0.015, 0.008, 0.025, 1.0)
