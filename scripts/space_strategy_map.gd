...
func _guard_passages() -> void:
	...
	var picked := 0
	var attempts := 0
	var wanted := STARTER_PASSAGE_GUARDIAN_COUNT if starter_map_mode else GUARDIAN_PASSAGE_COUNT
...
func _add_guardian(cell: Vector2i, template: String, site_index: int) -> void:
	...
	if kind == "trader":
		...
		guardian["aggro_radius"] = GUARDIAN_CONTROL_RADIUS
...
func _place_neutral_planets() -> void:
	...
	_clear_footprint_for_new_object(_footprint_cells(MapObjectDefs.TRADE_PLANET_CENTER, trade_size))
	_add_object_guardian(MapObjectDefs.TRADE_PLANET_CENTER, "trading_planet", trade_size)

	var pirate_size := MapObjectDefs.size("pirate_planet")
	_clear_footprint_for_new_object(_footprint_cells(MapObjectDefs.PIRATE_PLANET_CENTER, pirate_size))
	_add_object_guardian(MapObjectDefs.PIRATE_PLANET_CENTER, "pirate_planet", pirate_size)
...
func _clear_footprint_for_new_object(cells: Array[Vector2i]) -> void:
	for cell in cells:
		...
		if map_object_at.has(cell):
			...
			for occupied in _footprint_cells(object["cell"], int(object.get("size", 1))):
				map_object_at.erase(occupied)
		if guardian_at.has(cell):
			var guardian_index: int = guardian_at[cell]
			var guardian: Dictionary = guardians[guardian_index]
			guardian["alive"] = false
			for occupied in _footprint_cells(guardian["cell"], int(guardian.get("size", 1))):
...
func _generate_guarded_resource_caches() -> void:
	var pile_count := STARTER_RESOURCE_CACHE_COUNT if starter_map_mode \
...
func _add_resource_cache_guardian(cell: Vector2i, template: String) -> void:
	...
	guardians.append({
		...
		"patrol": true, "aggro_radius": GUARDIAN_CONTROL_RADIUS,
...
func _generate_wormhole_pairs() -> void:
	var pair_count := STARTER_WORMHOLE_PAIR_COUNT if starter_map_mode else MapObjectDefs.WORMHOLE_PAIR_COUNT
...
func _generate_production_sites() -> void:
	...
	_add_distant_production_sites(occupied_cells, STARTER_RARE_RESOURCE_COPIES if starter_map_mode else 3)
...
func _object_footprint_is_far_from_planets(anchor: Vector2i, size: int, minimum_distance: int) -> bool:
	for cell in _footprint_cells(anchor, size):
...
func _production_footprint_is_far_from_planets(anchor: Vector2i, minimum_distance: int) -> bool:
	for cell in _footprint_cells(anchor, PRODUCTION_FOOTPRINT.x):
		if _chebyshev_distance(cell, HUMAN_PLANET_CENTER) < minimum_distance \
			or _chebyshev_distance(cell, ORC_PLANET_CENTER) < minimum_distance:
			return false
	return true
