...
func _ready() -> void:
	...
	if OS.has_feature("mobile") and not DEMO.enabled():
		_button(column, "Управление", _show_mobile_help)
	if not DEMO.enabled():
...
func _show_demo_help() -> void:
	...
	add_child(dialog)
	var controls := "Карта: касание — маршрут, повторное касание той же клетки — полёт; свайп — камера, два пальца — масштаб, удержание на флоте — состав. Бой: первое касание показывает гекс, второе выполняет действие. Кнопка «Завершить сол» восстанавливает ход; кнопка Назад открывает сохранение и настройки." if OS.has_feature("mobile") else "ПКМ — проложить маршрут, пробел — продолжить полёт. WASD — камера, колесо — масштаб. Кнопка «Завершить сол» восстанавливает ход; Esc — сохранение и настройки."
	dialog.setup("МАРСИАНСКИЙ УЗЕЛ · ДЕМО", "Одна законченная миссия за полковника Павлову: освободите Марс, исследуйте станции и решите судьбу секретного архива. Контракты нейтралов необязательны.\n\n%s Журнал миссии хранит цели.\n\nКорабли нанимаются на Земле и на вольной верфи. Недельные станции пополняются в солы 1, 8, 15… Дополнительных героев в этой миссии нет." % controls)


func _show_mobile_help() -> void:
	var dialog := preload("res://scripts/object_reward_dialog.gd").new()
	add_child(dialog)
	dialog.setup("УПРАВЛЕНИЕ", "Карта: касание — маршрут, повторное касание той же клетки — полёт. Свайп перемещает камеру, два пальца меняют масштаб. Удержание на флоте открывает его состав.\n\nБой: первое касание показывает выбранный гекс и подсказку, второе выполняет действие. Способности, протоколы и завершение хода — кнопками внизу.\n\nКнопка Назад открывает сохранение и настройки; на планете сначала закрывает открытое окно.")
