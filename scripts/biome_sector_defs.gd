...
## Высокотемпературному лавовый ряд достаётся в своём прямом смысле: базальт с
## прожилками. Индекс 5 — лента пыли, её силуэт на месте глыбы не читается.
const LAVA_VARIANTS := [0, 1, 2, 3, 4]
## Обломки планеты: куски породы без техники. Тот же ряд, но без осколочного
## облака (3) — у разлетевшегося тела нужны крупные плиты, а не пыль.
const LAVA_SHARD_VARIANTS := [0, 1, 2]
## Выгоревшие остовы: то, что не пережило взрыв. Под рампой их металл
## становится раскалённым.
const SCORCHED_VARIANTS := [12, 13, 14, 15, 18, 19, 20, 21]
...
## У высокотемпературного почти нет разброса по цвету: жар задаёт рампа, а
## палитра только слегка меняет накал, чтобы соседние камни не были копиями.
const HOT_PALETTE := [
	[0.40, Color(0.80, 0.76, 0.74)],
	[0.72, Color(0.96, 0.92, 0.88)],
	[0.90, Color(1.04, 0.98, 0.92)],
	[1.0, Color(1.10, 0.88, 0.70)],
]
...
## * `accent` — что рисуется поверх: "glint" (блики на льду), "spore"
##   (споровые облака) или "ember" (угли и искры);
## * `shatter` — планетоиды собираются как разлетевшаяся планета, а не как
##   одно тело с кольцом; тогда нужен и `shard` — лист её обломков;
## * `molten` — цвет раскалённого нутра под крупными телами;
...
const PROFILES := {
	...
	"volcanic": {
		"node": "VolcanicSector",
		# Мелочь опять из ледяного листа: под рампой от синевы ничего не
		# остаётся, а нужны именно силуэты голой породы.
		"small": [ICE_SMALL, 8, 8],
		"small_variants": ROCK_VARIANTS,
		"grit_variants": ROCK_VARIANTS,
		"medium": [
			[SECTOR_PROPS, 6, 4, LAVA_VARIANTS],
			[SECTOR_PROPS, 6, 4, LAVA_VARIANTS],
			[SECTOR_PROPS, 6, 4, SCORCHED_VARIANTS],
		],
		"berg": [
			[SECTOR_PROPS, 6, 4, LAVA_VARIANTS],
			[SECTOR_PROPS, 6, 4, LAVA_SHARD_VARIANTS],
		],
		# Ориентир сектора — не отдельная картинка, а собранная композиция:
		# см. "shatter" ниже.
		"landmark": [],
		"shard": [SECTOR_PROPS, 6, 4, LAVA_SHARD_VARIANTS],
		"gas": [NEBULA, 3, 2],
		"vortex": [GALAXY, 2, 2],
		"palette": HOT_PALETTE,
		"shader": "ramp",
		# Рампа: тень — остывший базальт, полутон — тёмно-красная корка, блик —
		# раскалённый оранжевый. Прожилки лавы на листе как раз самые светлые,
		# поэтому они и достают до верха шкалы.
		"ramp": [Color(0.09, 0.07, 0.06), Color(0.34, 0.17, 0.11), Color(1.10, 0.62, 0.18)],
		"ramp_gamma": 1.35,
		"prop_tint": Color(1.0, 1.0, 1.0),
		"saturation": 1.0,
		"gas_tint": Color(0.52, 0.22, 0.09),
		"vortex_tint": Color(0.58, 0.26, 0.10),
		"haze_alpha": Vector2(0.09, 0.20),
		"gas_alpha": Vector2(0.22, 0.40),
		"vortex_alpha": Vector2(0.30, 0.48),
		# Взорванные планеты вместо обычных планетоидов и раскалённое нутро под
		# каждой крупной глыбой — см. biome_sector_renderer.gd.
		"shatter": true,
		"molten": Color(1.0, 0.44, 0.12),
		"accent": "ember",
	},
}
