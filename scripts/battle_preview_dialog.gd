...
## Пороги — не на глаз, а из замера настоящими боями (tools/balance_sim.gd,
## раздел 1b/1c; сведено в data/balance_plan.md §4): при ×1,0 игрок проигрывает
## 0 боёв из 9, при ×1,25 исход шаткий (7/9, но уцелевает мало флота), от ×1,5
## победа стабильна. Старая лесенка (2.0/1.2/0.8/0.5) называла ×1,0 "силы
## примерно равны", хотя это был гарантированный проигрыш — окно обманывало
## игрока в точности там, где ставки выше всего.
