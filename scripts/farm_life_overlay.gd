...
func _draw_greenhouse_lights() -> void:
	for index in range(GREENHOUSE_CENTERS.size()):
		...
		draw_circle(center, 20.0 + pulse * 8.0, Color(0.56, 1.0, 0.64, 0.025 + pulse * 0.025))
		draw_circle(center, 6.0 + pulse * 2.0, Color(0.96, 1.0, 0.75, 0.10 + pulse * 0.10))


func _draw_hub_beacon() -> void:
	var pulse := 0.5 + 0.5 * sin(elapsed * 1.3 + phase)
	var center := _texture_point(Vector2(0.50, 0.38))
	draw_circle(center, 13.0 + pulse * 4.0, Color(0.22, 0.78, 1.0, 0.04 + pulse * 0.05))
	draw_circle(center, 4.0, Color(0.57, 0.92, 1.0, 0.12 + pulse * 0.13))
