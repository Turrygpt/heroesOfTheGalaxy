...
## Постройки базы марсианских бандитов на глобальной карте: логова и форт появляются вокруг
## планеты по мере того, как ИИ их строит (см. bandit_ai.gd:_build). Как и другие
## оверлеи, состояния не держит — читает built_levels прямо из bandit_ai родителя.
##
...
## Смещение постройки от центра планеты марсианских бандитов, в клетках. Порядок и места
...
const BUILDING_OFFSETS := {
	...
	"fort": Vector2i(-2, 2),
	"marauder_fighter_yard": Vector2i(2, 2),
	"marauder_gunship_yard": Vector2i(-3, 0),
	"marauder_corvette_yard": Vector2i(3, 0),
	"marauder_frigate_yard": Vector2i(-2, -2),
	"marauder_destroyer_yard": Vector2i(2, -2),
}
...
func _draw() -> void:
	...
	if strategy_map.bandit_ai == null:
		return
	var built: Dictionary = strategy_map.bandit_ai.built_levels
	var center: Vector2i = strategy_map.BANDIT_PLANET_CENTER
	var cell_size: float = strategy_map.CELL_SIZE
	var accent: Color = OWNED_COLOR if int(strategy_map.bandit_planet_owner) == 2 else CAPTURED_COLOR
	for kind in BUILDING_OFFSETS:
		var level := int(built.get(kind, 0))
		if level <= 0:
			continue
		var texture := BanditDefs.building_texture(String(kind), level)
		if texture == null:
			continue
