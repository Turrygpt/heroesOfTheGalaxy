...
func _run() -> void:
	...
	# Обратная проверка: всё, что campaign_story.gd умеет ПОКАЗАТЬ, обязано
	# иметь реплики. Без неё правка сюжета уже уносила сценарии из справочника:
	# enqueue отмечал их показанными, intro_dialogue падал на пустом списке,
	# и карта навсегда оставалась в set_process(false).
	for id in ["intro", "supply", "gate", "ridus", "pirate_contract", "pirate_complete",
			"ridus_after", "refugees", "stein", "trader_contract", "trader_complete",
			"stein_after", "patrol_clearance", "ledger", "kowalski_ambush",
			"grak_battle", "archive", "relay", "approach", "mars_evidence", "ending"]:
