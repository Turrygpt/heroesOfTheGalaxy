...
func _run() -> void:
	...
	check(int(town.built_levels["fort"]) == 0, "Нельзя строить без денег")
	...
	for kind in town.MARS_UPGRADES:
		...
		for level in range(1, variants.size() + 1):
			check(town._find_catalog_texture(kind, level) is AtlasTexture, "Уровень вырезан из панорамы: %s %d" % [kind, level])
	...
	if trader_mode:
		...
		for id in weekly.available_growth:
			check(String(id).begins_with("league_"), "Лига производит только свои корабли")
		for id in UnitDefs.recruitable_ids("trader"):
	...
	check(reopened.placed_buildings.size() == (11 if trader_mode else 12), "Все здания фракции доступны")
	check(reopened.background.texture.resource_path.ends_with("master_v1.png" if faction == "mars" else "master_v3.png"), "Эталонная панорама фракции")
	check(reopened.state_patches.is_empty(), "При полной застройке оригинал не перекрыт")
	var centers := {
