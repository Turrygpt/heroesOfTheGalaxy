...
rem
rem Без video/intro.ogv сборка не делается молча: нужен явный --no-intro.
rem Такой сборке ролик
rem можно подложить и потом - файл intro.ogv рядом с exe игра найдёт сама.
setlocal

cd /d "%~dp0.."

set "PRESET=Windows Full"
set "REQUIRE_INTRO=1"
...
set "OUT=build/full/HeroesOfTheGalaxy.exe"
set "OUTDIR=build\full"
goto out_done
:out_web
...
echo Восстанови video\intro.ogv или положи нужный ролик в video\.
