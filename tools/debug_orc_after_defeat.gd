## Отладочный прогон: на 10-й сол убивает флот вождя орков (как настоящая
## победа игрока в бою) и смотрит, продолжает ли экономика орков жить —
## казна, стройка, найм, восстановление вождя и возврат к захвату месторождений.
## Не тест — запускается руками, см. tools/debug_orc_turns.gd для формата.
extends SceneTree
...
func _run() -> void:
	...
	for day in range(9):
		...
		ai.take_turn(map)
	print("--- сол 9 закончен, флот вождя: %s (built=%s, credits=%d) ---" % [str(warlord.army), str(ai.built_levels), ai.credits])
	print(">>> УБИВАЕМ ФЛОТ ВОЖДЯ (как после проигранного боя с игроком) <<<")
	ai.kill_hero(map)
	print("hero_alive=%s army=%s" % [str(ai.hero_alive), str(warlord.army)])
