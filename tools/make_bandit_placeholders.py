# -*- coding: utf-8 -*-
"""Генератор плейсхолдер-спрайтов марсианской фракции.

Рисует 10 кораблей (5 рангов x обычный/элитный), корабль главаря для карты и
здания марсианской базы. Настоящий арт заменит эти файлы позже — форматы и
...
* корабли смотрят ВЛЕВО (см. tactical_battle._draw_unit: "All source ships
  face left"), region в bandit_defs.gd = весь холст;
* корабль главаря смотрит ВВЕРХ (см. SHIP_SOURCE_ANGLE в space_strategy_map.gd);
* здания — квадрат 1254x1254, как у людей (assets/planet_surface/human).

Запуск:  python tools/make_bandit_placeholders.py
"""
...
SHIP_DIR = os.path.join(ROOT, "assets", "ships", "bandit")
BUILDING_DIR = os.path.join(ROOT, "assets", "planet_surface", "bandit")
...
def draw_ship(tier, elite):
	...
	# Орудийные блистеры: у марсианских бандитов оружие крупнее корпуса.
	for sign in (-1, 1):
...
def draw_hero_ship():
	"""Флагман главаря для глобальной карты: носом ВВЕРХ, квадратный холст."""
	size = 518
...
def draw_commander_portrait():
	"""Портрет главаря для панели стороны 2 в бою (см. tactical_battle_hud.gd).
...
# (см. bandit_defs.gd), поэтому у ангаров вместо цифры уровня метка ЭЛИТА.
...
def main():
	...
	for tier in range(1, 6):
		for elite in (False, True):
			...
			written.append(path)
	hero_path = os.path.join(HERO_DIR, "bandit.png")
	draw_hero_ship().save(hero_path)
	written.append(hero_path)
	portrait_path = os.path.join(PORTRAIT_DIR, "bandit_commander.png")
